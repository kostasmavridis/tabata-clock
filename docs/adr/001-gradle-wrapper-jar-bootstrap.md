# ADR-001: Bootstrap `gradle-wrapper.jar` at Runtime Instead of Committing It
...
- The hardcoded version will be caught by the `agp-gradle` Dependabot group (see ADR-003)
...
### Store the JAR as a GitHub Release asset and download it
Adds operational complexity with no security benefit over downloading from `services.gradle.org` directly.
